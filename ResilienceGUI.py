...
class ViewGraphs(QDialog):   
    ...
    def makemetriclist(self,basic,options):
        ...
        nodes_removed_A,node_count_removed_A,count_nodes_left_A,number_of_edges_A,number_of_components_A = basic
        size_of_components,giant_component_size,av_nodes_in_components,isolated_nodes,isolated_n_count,isolated_n_count_removed,subnodes,subnodes_count,path_length,av_path_length_components,giant_component_av_path_length,av_path_length_geo,average_degree,inter_removed_count = options
        ...
        if isolated_nodes <> False: #THIS NEEDS TO BE IN THE BASIC SET
            #not suitable for graphing
            pass
            #metric_list.append('Isolated nodes')
        if isolated_n_count <> False: #THIS NEEDS TO BE IN THE BASIC SET
...
class Window(QMainWindow):
    def __init__(self, parent = None):
        ...
        self.setWindowTitle('Network Tool v2.4') #title of window 
        self.setWindowIcon(QIcon('logo.png'))
        self.show() #show window
    ...
    def openConfig(self):
        ...
        there is an error in the config file, it notifes the user at the end 
        once changes are made which can be made.'''
        ...
        if fileName <> '':
            ...
            for item in line:
                ...
                #need some code here which handles the metircs and updating the states in the GUI
            ...
            ###
            #print 'multi here is:', self.multiiterations
            f.close()
        else:
            pass
        return
    ...
    def AtoBEdges(self):
        ...
        param3 = self.txtparamA3.text()        
        self.check_for_changes(param1,param2,param3,'A')
        if self.G == None or self.changedA == False:
            self.G = self.buildnetwork(param1,param2,param3, 'A') #builds network
            if self.G == None: return
        ...
        self.check_for_changes(param1,param2,param3,'A')
        if self.GnetB == None or self.changedA == False:
    ...
    def BtoAEdges(self):
        ...
        param2 = self.txtparamB2.text()
        param3 = self.txtparamB3.text()  
        self.check_for_changes(param1,param2,param3,'B')
        if self.GnetB == None or self.changedA == False:
            self.GnetB = self.buildnetwork(param1,param2,param3, 'B') #builds network
            if self.GnetB == None: return 
    ...
    def updateUi(self):
        ...
        print 'iterate is: ', self.iterate
        print ''
        print ''
        if self.iterationsdone > self.numofiterations+1:
            exit()
        if self.iterationsdone == self.numofiterations:
            print 'THEY ARE EQUAL'
        '''
        if int(self.iterationsdone) <> int(self.numofiterations) and self.iterate == False and self.multiiterations == True or self.runallseqmodels == True:
            print 'this does not work'
        else:
            print 'THiS is why it does not work atm'
            exit()
        '''
        ...
        print 'iterate now is: ', self.iterate
        print 'iterations done: ', self.iterationsdone 
        print 'iterations to do: ', self.numofiterations
        ...
        if self.ckbxviewnet.isChecked() or self.saveimage == True:
            ...
            if self.timestep > -1:
                ...
                if self.multiiterations == True:
                    ...
                    self.figureModel, self.timestep, self.whenToSave = draw(self.graphvis, self.positions, self.figureModel, self.timestep, self.coloractive, self.colorinactive, show, self.pertimestep, self.imagedestlocation, self.whenToSave)
        ...
        #for the end of a multiple iteration analysis
            ...
            if self.saveoutputfile == True and self.multiiterations == True:
                ...
                f.write('\nAverage values are:\n')
                f.close()
                #print 'sending stuff for average:', self.multiiterations
                logfilepath = None
                if self.runallseqmodels == True:                
                    if RANDOM == True:
                        fileName = fileName+'_RANDOM'
                        ...
                        fileName = fileName+'_BETWEENNESS'
                    parameters = metrics, STAND_ALONE, DEPENDENCY, INTERDEPENDENCY, SINGLE, SEQUENTIAL, CASCADING, RANDOM, DEGREE, BETWEENNESS, REMOVE_SUBGRAPHS, REMOVE_ISOLATES, NO_ISOLATES, fileName,a_to_b_edges
                    self.values, error = res.outputresults(self.graphparameters, parameters,logfilepath,self.multiiterations)
                else:
                    self.values, error = res.outputresults(self.graphparameters, self.parameters,logfilepath,self.multiiterations)
                if error <> None:
                    if error == 0045:
                        ...
                        return
                #print'need method to calc averages'
            ...
            '''
            print 'checking to see if need to run again'
            if self.runallseqmodels == True and BETWEENNESS <> True:
                self.runseqmodels()
            else:
                print 'number of iterations done is: ', self.numofiterations 
                ok = QMessageBox.information(self, 'Information', "Network resileince analysis successfully completed. Do you want to view the metric graphs?" , '&No','&View Graphs')
                if ok == 1: #if the view graph option is clicked
    ...
    def drawview(self):
        ...
        param3 = self.txtparamA3.text()
        self.check_for_changes(param1,param2,param3, 'A')
        if self.graph == 'Database': #user can't edit the network if from the database, and when loaded, it is built
            pass
        else:        
            if self.G == None or self.changedA == True: #checks if the user has changed any of the input values
                print 'NEED TO BUILD NETWORK'                
                self.positions = None #if so, need toreset the positions as well
                self.G = self.buildnetwork(param1,param2,param3,'A')
                if self.G == None:
                    return
        ...
        if selected == False: #if the user clicks cancel
            self.graphvis = None
            return
    ...
    def step_analysis(self):
        ...
        if self.ckbxviewnet.isChecked() or self.saveimage == True and self.positions==None: 
            selected = self.show_visselection()
            self.positions = self.getpositions(selected)
    ...
    def full_analysis(self, parameters = None):
        ...
        if self.timestep > 0:
            ...
            #print 'GtempA has nodes: ', GtempA.number_of_nodes()
            self.G = GA     
            self.thread.setup(self.G, self.iterate, self.parameters, self.graphparameters)
    ...
    def get_analysis_type(self):
        ...
        print 'ANALYSIS TYPE: ', self.analysistype
    ...
    def buildnetwork(self, param1,param2,param3, net):
        ...
        self.param2 = param2
        self.param3 = param3
        print 'before returning network as %s nodes'%(tempG.number_of_nodes())
        if tempG == None:
            self.cancel = True
            QMessageBox.warning(self, 'Error!', "Error! Apollogies, the cause is unknown.")
            return
...
def draw(G, positions, figureModel, timestep, coloractive, colorinactive, show, pertimestep,imagedestlocation,whenToSave):
